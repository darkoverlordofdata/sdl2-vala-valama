...
void hello_world_run (HelloWorld* self) {
	...
	while (TRUE) {
		...
		SDL_SetRenderDrawColor (_tmp13_, (guint8) 0xFF, (guint8) 0xFF, (guint8) 0xFF, (guint8) SDL_ALPHA_OPAQUE);
		_tmp14_ = self->priv->renderer;
		SDL_RenderClear (_tmp14_);
		_tmp15_ = self->priv->arrowTexture;
		_tmp16_ = self->priv->renderer;
		_tmp17_ = self->priv->arrowTexture;
		_tmp18_ = my_texture_get_width (_tmp17_);
		_tmp19_ = _tmp18_;
		_tmp20_ = self->priv->arrowTexture;
		_tmp21_ = my_texture_get_height (_tmp20_);
		_tmp22_ = _tmp21_;
		_tmp23_ = degrees;
		_tmp24_ = flipType;
		my_texture_render (_tmp15_, _tmp16_, (HELLO_WORLD_SCREEN_WIDTH - _tmp19_) / 2, (HELLO_WORLD_SCREEN_HEIGHT - _tmp22_) / 2, NULL, _tmp23_, NULL, _tmp24_);
...
gboolean hello_world_load_media (HelloWorld* self) {
	...
	_tmp1_ = my_texture_from_file (_tmp0_, "resources/arrow.png");
	_g_object_unref0 (self->priv->arrowTexture);
	self->priv->arrowTexture = _tmp1_;
	_tmp2_ = self->priv->arrowTexture;
	if (_tmp2_ == NULL) {
		FILE* _tmp3_ = NULL;
		_tmp3_ = stdout;
		fputs ("Failet to load arrow texture!\n", _tmp3_);
		result = FALSE;
		return result;
	}
	result = TRUE;
