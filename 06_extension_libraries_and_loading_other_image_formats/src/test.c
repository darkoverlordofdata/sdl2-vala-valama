		...
		while (TRUE) {
			...
			if (_tmp4_ == SDL_QUIT) {
				...
				if (_tmp6_ == SDL_KEYDOWN) {
					...
					_tmp11_ = _tmp10_.sym;
					_tmp12_ = _sdl_keycode_to_string (_tmp11_);
					fprintf (_tmp7_, "%s\n", _tmp12_);
				}
			}
			_tmp13_ = self->priv->pngSurface;
			_tmp14_ = self->priv->surface;
			SDL_BlitSurface (_tmp13_, NULL, _tmp14_, NULL);
			_tmp15_ = self->priv->window;
			SDL_UpdateWindowSurface (_tmp15_);
		}
	}
	 (e);
}
...
void hello_world_load_media (HelloWorld* self) {
	...
	SDL_PixelFormat* _tmp2_ = NULL;
	SDL_Surface* _tmp3_ = NULL;
	g_return_if_fail (self != NULL);
	_tmp0_ = IMG_Load ("resources/loaded.png");
	s = _tmp0_;
	_tmp1_ = self->priv->surface;
	_tmp2_ = _tmp1_->format;
	_tmp3_ = SDL_ConvertSurface (s, _tmp2_, (guint32) 0);
	_SDL_FreeSurface0 (self->priv->pngSurface);
	self->priv->pngSurface = _tmp3_;
	_SDL_FreeSurface0 (s);
}
