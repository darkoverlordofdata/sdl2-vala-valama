...
void hello_world_run (HelloWorld* self) {
	...
	while (TRUE) {
		...
		topLeftViewport = _tmp7_;
		_tmp8_ = self->priv->renderer;
		_tmp9_ = topLeftViewport;
		SDL_RenderSetViewport (_tmp8_, &_tmp9_);
		_tmp10_ = self->priv->renderer;
		_tmp11_ = self->priv->texture;
		SDL_RenderCopy (_tmp10_, _tmp11_, NULL, NULL);
		_tmp12_.x = HELLO_WORLD_SCREEN_WIDTH / 2;
		_tmp12_.y = 0;
		_tmp12_.w = HELLO_WORLD_SCREEN_WIDTH / 2;
		_tmp12_.h = HELLO_WORLD_SCREEN_HEIGHT / 2;
		topRightViewport = _tmp12_;
		_tmp13_ = self->priv->renderer;
		_tmp14_ = topRightViewport;
		SDL_RenderSetViewport (_tmp13_, &_tmp14_);
		_tmp15_ = self->priv->renderer;
		_tmp16_ = self->priv->texture;
		SDL_RenderCopy (_tmp15_, _tmp16_, NULL, NULL);
		_tmp17_.x = 0;
		_tmp17_.y = HELLO_WORLD_SCREEN_HEIGHT / 2;
		_tmp17_.w = HELLO_WORLD_SCREEN_WIDTH;
		_tmp17_.h = HELLO_WORLD_SCREEN_HEIGHT / 2;
		bottomViewport = _tmp17_;
		_tmp18_ = self->priv->renderer;
		_tmp19_ = bottomViewport;
		SDL_RenderSetViewport (_tmp18_, &_tmp19_);
		_tmp20_ = self->priv->renderer;
		_tmp21_ = self->priv->texture;
		SDL_RenderCopy (_tmp20_, _tmp21_, NULL, NULL);
		_tmp22_ = self->priv->renderer;
		SDL_RenderPresent (_tmp22_);
	}
	 (e);
}
...
gboolean hello_world_load_media (HelloWorld* self) {
	...
	g_return_val_if_fail (self != NULL, FALSE);
	_tmp0_ = hello_world_loadTexture (self, "resources/viewport.png");
	...
	if (_tmp1_ == NULL) {
		FILE* _tmp2_ = NULL;
		_tmp2_ = stdout;
		fputs ("Failet to load texture image!\n", _tmp2_);
		result = FALSE;
		return result;
	}
	result = TRUE;
