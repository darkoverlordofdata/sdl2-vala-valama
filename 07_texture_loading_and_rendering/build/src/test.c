...
void hello_world_run (HelloWorld* self) {
	...
	while (TRUE) {
		...
		SDL_RenderClear (_tmp5_);
		_tmp6_ = self->priv->renderer;
		_tmp7_ = self->priv->texture;
		SDL_RenderCopy (_tmp6_, _tmp7_, NULL, NULL);
		_tmp8_ = self->priv->renderer;
		SDL_RenderPresent (_tmp8_);
	}
	 (e);
}
...
gboolean hello_world_init (HelloWorld* self) {
	...
	_tmp8_ = SDL_CreateWindow ("SDL Tutorial", (gint) SDL_WINDOWPOS_CENTERED_MASK, (gint) SDL_WINDOWPOS_CENTERED_MASK, 640, 480, (guint32) SDL_WINDOW_SHOWN);
...
gboolean hello_world_load_media (HelloWorld* self) {
	...
	g_return_val_if_fail (self != NULL, FALSE);
	_tmp0_ = hello_world_loadTexture (self, "resources/texture.png");
	...
	if (_tmp1_ == NULL) {
		FILE* _tmp2_ = NULL;
		_tmp2_ = stdout;
		fputs ("Failed to load texture image!\n", _tmp2_);
		result = FALSE;
		return result;
	}
	result = TRUE;
