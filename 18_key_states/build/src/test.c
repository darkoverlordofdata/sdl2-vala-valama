...
struct _HelloWorldPrivate {
	...
	MyTexture* pressTexture;
	MyTexture* upTexture;
	MyTexture* downTexture;
	MyTexture* leftTexture;
	MyTexture* rightTexture;
};
...
void hello_world_run (HelloWorld* self) {
	gboolean quit = FALSE;
	SDL_Event e = {0};
	MyTexture* currentTexture = NULL;
	g_return_if_fail (self != NULL);
	quit = FALSE;
	while (TRUE) {
		gboolean _tmp0_ = FALSE;
		guint8* currentKeyStates = NULL;
		gint _tmp5_ = 0;
		guint8* _tmp6_ = NULL;
		gint currentKeyStates_length1 = 0;
		gint _currentKeyStates_size_ = 0;
		guint8* _tmp7_ = NULL;
		gint _tmp7__length1 = 0;
		guint8 _tmp8_ = 0U;
		SDL_Renderer* _tmp25_ = NULL;
		SDL_Renderer* _tmp26_ = NULL;
		MyTexture* _tmp27_ = NULL;
		...
		_tmp6_ = SDL_GetKeyboardState (&_tmp5_);
		currentKeyStates = _tmp6_;
		currentKeyStates_length1 = _tmp5_;
		_currentKeyStates_size_ = currentKeyStates_length1;
		_tmp7_ = currentKeyStates;
		_tmp7__length1 = currentKeyStates_length1;
		_tmp8_ = _tmp7_[SDL_SCANCODE_UP];
		if (((gint) _tmp8_) == 1) {
			MyTexture* _tmp9_ = NULL;
			MyTexture* _tmp10_ = NULL;
			_tmp9_ = self->priv->upTexture;
			_tmp10_ = _g_object_ref0 (_tmp9_);
			_g_object_unref0 (currentTexture);
			currentTexture = _tmp10_;
		} else {
			guint8* _tmp11_ = NULL;
			gint _tmp11__length1 = 0;
			guint8 _tmp12_ = 0U;
			_tmp11_ = currentKeyStates;
			_tmp11__length1 = currentKeyStates_length1;
			_tmp12_ = _tmp11_[SDL_SCANCODE_DOWN];
			if (((gint) _tmp12_) == 1) {
				MyTexture* _tmp13_ = NULL;
				MyTexture* _tmp14_ = NULL;
				_tmp13_ = self->priv->downTexture;
				_tmp14_ = _g_object_ref0 (_tmp13_);
				_g_object_unref0 (currentTexture);
				currentTexture = _tmp14_;
			} else {
				guint8* _tmp15_ = NULL;
				gint _tmp15__length1 = 0;
				guint8 _tmp16_ = 0U;
				_tmp15_ = currentKeyStates;
				_tmp15__length1 = currentKeyStates_length1;
				_tmp16_ = _tmp15_[SDL_SCANCODE_LEFT];
				if (((gint) _tmp16_) == 1) {
					MyTexture* _tmp17_ = NULL;
					MyTexture* _tmp18_ = NULL;
					_tmp17_ = self->priv->leftTexture;
					_tmp18_ = _g_object_ref0 (_tmp17_);
					_g_object_unref0 (currentTexture);
					currentTexture = _tmp18_;
				} else {
					guint8* _tmp19_ = NULL;
					gint _tmp19__length1 = 0;
					guint8 _tmp20_ = 0U;
					_tmp19_ = currentKeyStates;
					_tmp19__length1 = currentKeyStates_length1;
					_tmp20_ = _tmp19_[SDL_SCANCODE_RIGHT];
					if (((gint) _tmp20_) == 1) {
						MyTexture* _tmp21_ = NULL;
						MyTexture* _tmp22_ = NULL;
						_tmp21_ = self->priv->rightTexture;
						_tmp22_ = _g_object_ref0 (_tmp21_);
						_g_object_unref0 (currentTexture);
						currentTexture = _tmp22_;
					} else {
						MyTexture* _tmp23_ = NULL;
						MyTexture* _tmp24_ = NULL;
						_tmp23_ = self->priv->pressTexture;
						_tmp24_ = _g_object_ref0 (_tmp23_);
						_g_object_unref0 (currentTexture);
						currentTexture = _tmp24_;
					}
				}
			}
		}
		_tmp25_ = self->priv->renderer;
		SDL_SetRenderDrawColor (_tmp25_, (guint8) 0xFF, (guint8) 0xFF, (guint8) 0xFF, (guint8) SDL_ALPHA_OPAQUE);
		_tmp26_ = self->priv->renderer;
		SDL_RenderClear (_tmp26_);
		_tmp27_ = currentTexture;
		_tmp28_ = self->priv->renderer;
		my_texture_render (_tmp27_, _tmp28_, 0, 0, NULL, 0.0, NULL, SDL_FLIP_NONE);
		_tmp29_ = self->priv->renderer;
		SDL_RenderPresent (_tmp29_);
	}
	_g_object_unref0 (currentTexture);
	 (e);
}
...
gboolean hello_world_load_media (HelloWorld* self) {
	...
	_tmp1_ = my_texture_from_file (_tmp0_, "resources/press.png");
	_g_object_unref0 (self->priv->pressTexture);
	self->priv->pressTexture = _tmp1_;
	_tmp2_ = self->priv->pressTexture;
	if (_tmp2_ == NULL) {
		FILE* _tmp3_ = NULL;
		_tmp3_ = stdout;
		fputs ("Failed to load press texture!\n", _tmp3_);
		result = FALSE;
		return result;
	}
	_tmp4_ = self->priv->renderer;
	_tmp5_ = my_texture_from_file (_tmp4_, "resources/up.png");
	_g_object_unref0 (self->priv->upTexture);
	self->priv->upTexture = _tmp5_;
	_tmp6_ = self->priv->upTexture;
	if (_tmp6_ == NULL) {
		FILE* _tmp7_ = NULL;
		_tmp7_ = stdout;
		fputs ("Failed to load up texture!\n", _tmp7_);
		result = FALSE;
		return result;
	}
	_tmp8_ = self->priv->renderer;
	_tmp9_ = my_texture_from_file (_tmp8_, "resources/down.png");
	_g_object_unref0 (self->priv->downTexture);
	self->priv->downTexture = _tmp9_;
	_tmp10_ = self->priv->downTexture;
	if (_tmp10_ == NULL) {
		FILE* _tmp11_ = NULL;
		_tmp11_ = stdout;
		fputs ("Failed to load down texture!\n", _tmp11_);
		result = FALSE;
		return result;
	}
	_tmp12_ = self->priv->renderer;
	_tmp13_ = my_texture_from_file (_tmp12_, "resources/left.png");
	_g_object_unref0 (self->priv->leftTexture);
	self->priv->leftTexture = _tmp13_;
	_tmp14_ = self->priv->leftTexture;
	if (_tmp14_ == NULL) {
		FILE* _tmp15_ = NULL;
		_tmp15_ = stdout;
		fputs ("Failed to load left texture!\n", _tmp15_);
		result = FALSE;
		return result;
	}
	_tmp16_ = self->priv->renderer;
	_tmp17_ = my_texture_from_file (_tmp16_, "resources/right.png");
	_g_object_unref0 (self->priv->rightTexture);
	self->priv->rightTexture = _tmp17_;
	_tmp18_ = self->priv->rightTexture;
	if (_tmp18_ == NULL) {
		FILE* _tmp19_ = NULL;
		_tmp19_ = stdout;
		fputs ("Failed to load right texture!\n", _tmp19_);
		result = FALSE;
		return result;
	}
	result = TRUE;
