...
#define HELLO_WORLD_WALKING_ANIMATION_FRAMES 4
...
void hello_world_run (HelloWorld* self) {
	...
	while (TRUE) {
		...
		_tmp7_ = frame;
		_tmp8_ = self->priv->spriteClips[_tmp7_ / 4];
		currentClip = _tmp8_;
		_tmp9_ = self->priv->spriteSheetTexture;
		...
		_tmp14_ = _tmp13_.h;
		_tmp15_ = currentClip;
		my_texture_render (_tmp9_, _tmp10_, (HELLO_WORLD_SCREEN_WIDTH - _tmp12_) / 2, (HELLO_WORLD_SCREEN_HEIGHT - _tmp14_) / 2, &_tmp15_);
		_tmp16_ = self->priv->renderer;
		SDL_RenderPresent (_tmp16_);
		_tmp17_ = frame;
		frame = _tmp17_ + 1;
		_tmp18_ = frame;
		if ((_tmp18_ / 4) >= HELLO_WORLD_WALKING_ANIMATION_FRAMES) {
			frame = 0;
		}
	}
	 (e);
...
gboolean hello_world_load_media (HelloWorld* self) {
	...
	_g_object_unref0 (self->priv->spriteSheetTexture);
	self->priv->spriteSheetTexture = _tmp1_;
	_tmp2_ = self->priv->spriteSheetTexture;
	if (_tmp2_ == NULL) {
		FILE* _tmp3_ = NULL;
		_tmp3_ = stdout;
		fputs ("Failet to load walking animation texture!\n", _tmp3_);
		result = FALSE;
		return result;
	} else {
		SDL_Rect _tmp4_ = {0};
		...
		_tmp4_.w = 64;
		_tmp4_.h = 205;
		self->priv->spriteClips[0] = _tmp4_;
		_tmp5_ = self->priv->spriteClips[0];
		_tmp6_.x = 64;
		_tmp6_.y = 0;
		_tmp6_.w = 64;
		_tmp6_.h = 205;
		self->priv->spriteClips[1] = _tmp6_;
		_tmp7_ = self->priv->spriteClips[1];
		_tmp8_.x = 128;
		_tmp8_.y = 0;
		_tmp8_.w = 64;
		_tmp8_.h = 205;
		self->priv->spriteClips[2] = _tmp8_;
		_tmp9_ = self->priv->spriteClips[2];
		_tmp10_.x = 196;
		_tmp10_.y = 0;
		_tmp10_.w = 64;
		_tmp10_.h = 205;
		self->priv->spriteClips[3] = _tmp10_;
		_tmp11_ = self->priv->spriteClips[3];
	}
	result = TRUE;
	return result;
}
