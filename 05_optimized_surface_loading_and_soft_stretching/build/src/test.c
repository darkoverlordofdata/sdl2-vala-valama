...
void hello_world_run (HelloWorld* self) {
	...
	while (TRUE) {
		...
		while (TRUE) {
			...
			rect.w = 640;
			rect.h = 480;
			_tmp13_ = self->priv->stretchedSurface;
			_tmp14_ = self->priv->surface;
			_tmp15_ = rect;
			SDL_BlitScaled (_tmp13_, NULL, _tmp14_, &_tmp15_);
			_tmp16_ = self->priv->window;
			SDL_UpdateWindowSurface (_tmp16_);
		}
	}
	 (e);
}
...
void hello_world_load_media (HelloWorld* self) {
	...
	SDL_PixelFormat* _tmp1_ = NULL;
	SDL_Surface* _tmp2_ = NULL;
	g_return_if_fail (self != NULL);
	_tmp0_ = SDL_LoadBMP ("resources/stretch.bmp");
	surface = _tmp0_;
	_tmp1_ = surface->format;
	_tmp2_ = SDL_ConvertSurface (surface, _tmp1_, (guint32) 0);
	_SDL_FreeSurface0 (self->priv->stretchedSurface);
	self->priv->stretchedSurface = _tmp2_;
